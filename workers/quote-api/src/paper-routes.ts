...
function paperPrecheck(env: Env, origin: string, requireOpenMarket: boolean): Response | null {
  ...
  if (requireOpenMarket) {
    ...
    if (!session.isOpen) return json({ error: 'MARKET_SESSION_CLOSED', status: session.status, asOf: session.asOf, timeZone: session.timeZone }, 409, origin);
  }
  return null;
}

function isInternalStateUnavailable(error: unknown): boolean { return error instanceof Error && error.message === INTERNAL_STATE_UNAVAILABLE; }
function isDailyLossUnavailable(error: unknown): boolean { return error instanceof Error && error.message === 'KIS_DAILY_LOSS_UNAVAILABLE:PAPER'; }

export async function handlePaperOrder({ request, env, origin }: RouteContext): Promise<Response> {
  const blocked = paperPrecheck(env, origin, true); if (blocked) return blocked;
  ...
  try { adapter.assertSubmittable(orderRequest); } catch (error) {
  ...
  try {
    ...
    const [reconciliation, quote, killSwitch, dailyLossResult] = await Promise.all([
      reconcileWithKis(env, internalState), createQuoteAdapter(env).getQuote(orderRequest.symbol), readKillSwitch(env),
      createAccountAdapter(env).getDailyLoss(todayKst()).then((snapshot) => ({ dailyLoss: snapshot.dailyLoss, available: true })).catch((error) => {
        if (isDailyLossUnavailable(error)) return { dailyLoss: 0, available: false }; throw error;
      }),
    ]);
    ...
    const risk = checkRisk({ request: orderRequest, state: { positions: internalState.positions, orders: records, dailyLoss: dailyLossResult.dailyLoss, dailyLossAvailable: dailyLossResult.available }, market: { referencePrice, quoteAsOf: quoteReferenceTime(quote), now }, config: DEFAULT_RISK_CONFIG, killSwitchActive: killSwitch.active, reconciliationAllowed: reconciliation.canPlaceNewOrders, apiHealthy: true });
  ...
  try { submission = await adapter.submit(order); } catch (error) {
    console.error('PAPER order submission entered UNKNOWN', errorMessage(error));
    try { order = transitionOrder(order, 'UNKNOWN'); await applyInternalOrder(env, order); } catch (persistError) { return errorResponse(persistError, origin, 'RECONCILIATION'); }
    return json({ error: 'PAPER_ORDER_UNKNOWN', order }, 502, origin);
  }
  try {
    if (!submission.accepted) { order = transitionOrder(order, 'REJECTED'); await applyInternalOrder(env, order); return json({ error: 'PAPER_ORDER_REJECTED', code: submission.messageCode, message: submission.message, order }, 409, origin); }
    order = toPaperAcceptedOrder(order, submission); await applyInternalOrder(env, order);
    return json({ order, messageCode: submission.messageCode, message: submission.message }, 200, origin);
  } catch (error) {
    console.error('PAPER order accepted by broker but internal state was not persisted', { brokerOrderId: submission.brokerOrderId, clientOrderId: orderRequest.clientOrderId, message: errorMessage(error) });
...
export async function handlePaperCancel({ request, env, origin }: RouteContext): Promise<Response> {
  ...
  try {
    const internalState = await readInternalState(env);
    order = (internalState.orderRecords ?? []).find((item) => (typeof id === 'string' && item.id === id) || (typeof clientOrderId === 'string' && item.clientOrderId === clientOrderId));
    ...
    if (!cancellation.accepted) return json({ error: 'PAPER_ORDER_CANCEL_REJECTED', code: cancellation.messageCode, message: cancellation.message, order }, 409, origin);
    const cancelPending = transitionOrder(order, 'CANCEL_PENDING');
    await applyInternalOrder(env, cancelPending);
    return json({ order: cancelPending, messageCode: cancellation.messageCode, message: cancellation.message }, 200, origin);
  } catch (error) {
    if (order && error instanceof KISHttpError) {
      try { await applyInternalOrder(env, transitionOrder(order, 'UNKNOWN')); } catch (persistError) { return errorResponse(persistError, origin, 'RECONCILIATION'); }
    }
...
export async function handlePaperPositionSync({ env, origin }: RouteContext): Promise<Response> {
  const blocked = paperPrecheck(env, origin, false); if (blocked) return blocked;
  try {
    const account = await createAccountAdapter(env).getSnapshot(); const previous = await readInternalState(env);
    const positions = account.positions.map(({ symbol, quantity }) => ({ symbol, quantity })); const state = await syncInternalPositions(env, positions);
    return json({ asOf: new Date().toISOString(), environment: getEnvironment(env), source: account.source, previousPositions: previous.positions, positions: state.positions, positionCount: state.positions.length, syncedAt: state.updatedAt }, 200, origin);
  } catch (error) { return errorResponse(error, origin, 'POSITION_SYNC'); }
}

export async function handlePaperReconcile({ env, origin }: RouteContext): Promise<Response> {
  const blocked = paperPrecheck(env, origin, false); if (blocked) return blocked;
  try {
    const today = todayKst(); const [internalState, account, brokerHistory] = await Promise.all([readInternalState(env), createAccountAdapter(env).getSnapshot(), createOrderAdapter(env).getOrderHistory(today, today)]);
    const internalOrders = internalState.orderRecords ?? []; const result = resyncPaperOrders(internalOrders, brokerHistory.orders);
    for (const [index, updatedOrder] of result.orders.entries()) if (updatedOrder !== internalOrders[index]) await applyInternalOrder(env, updatedOrder);
    return json({ asOf: new Date().toISOString(), environment: getEnvironment(env), account, updated: result.updated, unresolved: result.unresolved, orders: result.orders }, 200, origin);
  } catch (error) { return errorResponse(error, origin, 'RECONCILIATION'); }
}
