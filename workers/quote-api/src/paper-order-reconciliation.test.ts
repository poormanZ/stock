...
const submitted = () => transitionOrder({ ...base, brokerOrderId: 'b1', brokerOrderOrgNo: 'org1' }, 'SUBMITTING');
const accepted = () => transitionOrder(submitted(), 'SUBMITTED');

describe('PAPER order reconciliation', () => {
  it('moves an accepted order to PARTIALLY_FILLED with broker execution data', () => {
    const order = transitionOrder(accepted(), 'ACCEPTED');
    const result = resyncPaperOrders([order], [broker({ executedQuantity: 4, averageExecutedPrice: 70100, status: 'PARTIALLY_FILLED' })]);
  ...
  it('moves a partially filled order to FILLED only when the full quantity executed', () => {
    const partial = transitionOrder(accepted(), 'PARTIALLY_FILLED', { executedQuantity: 4, averageExecutedPrice: 70100 });
  ...
  it('rejects impossible execution quantities', () => {
    const order = transitionOrder(accepted(), 'ACCEPTED');
    expect(() => resyncPaperOrders([order], [broker({ executedQuantity: 10, status: 'PARTIALLY_FILLED' })])).toThrow('INVALID_PARTIAL_FILL_QUANTITY');
    expect(() => resyncPaperOrders([order], [broker({ executedQuantity: 9, status: 'FILLED' })])).toThrow('FILLED_QUANTITY_MISMATCH');
  });

  it('is idempotent for an unchanged broker snapshot', () => {
    const order = transitionOrder(accepted(), 'ACCEPTED');
    const first = resyncPaperOrders([order], [broker({ executedQuantity: 4, averageExecutedPrice: 70100, status: 'PARTIALLY_FILLED' })]);
