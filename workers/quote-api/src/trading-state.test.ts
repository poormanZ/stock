...
const config = { mode: 'DRY_RUN', symbols: ['005930'], strategy: { id: 'sma-crossover' }, candleBars: 260 };
...
describe('trading state', () => {
  it('validates configs and fills defaults', () => {
    ...
    expect(valid).toMatchObject({ mode: 'DRY_RUN', symbols: ['005930'], strategy: { id: 'sma-crossover', params: { fast: 10, slow: 30, trend: 200 } }, candleBars: 260 });
    // candleBars를 생략하면 워밍업(201) + 여유로 정한다
    expect(validateTradingConfig({ ...config, candleBars: undefined }).candleBars).toBe(211);
    expect(validateTradingConfig({ ...config, candleBars: undefined, strategy: { id: 'sma-crossover', params: { fast: 5, slow: 20, trend: 0 } } }).candleBars).toBe(60);
