...
```json
{
  ...
  "volume": 16936614,
  "asOf": "",
  "fetchedAt": "2026-09-14T06:30:12.000Z",
  "market": "KRX",
  "source": "KIS OPEN API",
  "delayed": false
}
...
- `asOf`: KIS가 돌려주는 마지막 체결시각(`YYYYMMDD HHMMSS`, KST). 현재가 API(`inquire-price`)는 체결시각 필드를 제공하지 않아 운영 환경에서는 빈 문자열이다.
- `fetchedAt`: Worker가 KIS로부터 시세를 수신한 시각(ISO 8601, UTC). 항상 존재한다.
...
7. stale/missing 상태는 Risk Manager에서 최종 주문 차단 조건으로 재사용한다. 주문 판단용 시세 기준 시각은 `asOf`를 ISO로 변환한 값을 우선 사용하고, `asOf`가 비어 있으면 `fetchedAt`(수신 시각)을 사용한다. PAPER 주문 경로는 정규장 시간 검사를 먼저 통과해야 하므로, 수신 시각 기준 15초 이내 시세만 주문 판단에 쓰인다.

이 정책은 **표시용 시세와 주문 판단용 시세를 분리**하기 위한 것이다. 현재 Worker의 `KIS_ENVIRONMENT`는 `LIVE`(조회 전용)이며, LIVE 주문 API는 구현하지 않는다.
